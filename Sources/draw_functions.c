...
///----------------------------Fun��es de desenho-----------------------------
    ...
    void draw_hero(HERO hero, SRC media){
        ...
        if(hero.afterDeath){
            ...
            //S� se mexe se estiver apertando a tecla
    ...
    void draw_enm1(ENEMY enm1, SRC media){
        ...
        //S� se mexe se n�o estiver no modo vulner�vel
        if(enm1.status!=1){
            framesCounter2++;
...
int two_answers(SRC media, char question[maxString], char opt1[maxString], char opt2[maxString]){
    Vector2 coordTitle = (Vector2){400,275}; //Coordenada do t�tulo
    int selected=0;
    int fontSize=40;
